...
#SBATCH --job-name=c_bb_1st
#SBATCH --output=class_bb_1st_half.txt
...
singularity run --nv \
    ...
    --mine /opt/data_dir/promath/math{00..17}/*.tar.gz
cp -r /tmp/trainer $PROJECT/$OUTDIRNAME/trainer_logs
cp ./class_bb_1st_half.txt $PROJECT/$OUTDIRNAME/trainer_logs
