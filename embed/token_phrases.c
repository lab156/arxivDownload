...
int AddWordToVocab(char *word) {
  ...
  //printf("Adding word %s with length: %i\n", word, length);
...
int main(int argc, char **argv) {
    /*
    char palabra[MAX_STRING], *eof=0;
    FILE *fptr;
    fptr = fopen("/home/luis/Documents/arxivDownload/data/leer_archivo.txt", "r");
    printf("Allocating the vocab_word memory\n");
  vocab = (struct vocab_word *)calloc(vocab_max_size, sizeof(struct vocab_word));
    printf("allocation the vocab_has memory\n");
  vocab_hash = (int *)calloc(vocab_hash_size, sizeof(int));
  for (int a = 0; a < vocab_hash_size; a++) vocab_hash[a] = -1;
  vocab_size = 0;
  AddWordToVocab((char *)"</s>");
  printf("Done memory allocation!\n");
    ReadWord(palabra, fptr, eof);
    printf("Palabra is %s\n", palabra );
    int hash = GetWordHash(palabra);
    printf("Hash is %i\n", hash );
    int indice = AddWordToVocab(palabra);
    ReadWord(palabra, fptr, eof);
    AddWordToVocab(palabra);
    printf("Palabra is %s, and it's hash is: %i and indice=%i\n", palabra, hash, indice);
    int ind = 2;
    printf(" Para el indice %i la palabra es: %s\n", ind, vocab[ind].word);
    fclose(fptr);
    */
  int i;
  if (argc == 1) {
